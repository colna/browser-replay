...
- **点击上溯到可交互祖先。** 点一个按钮，事件的 `target` 往往是按钮里的 `<span><span>文字</span></span>`，这些包裹层没有任何标识，只能退到 `nav > div > … > span > span > span` 这种十来层路径；而外面那个按钮多半带着 `aria-label` 或 `data-testid`。所以目标自身只能退到结构路径时，会向上找最近的**可交互元素**（按钮 / 链接 / `role=button`），且必须确实更好认才换。只上溯到可交互元素而不是布局容器——回放点击取元素中心点，容器太大时中心可能落到别的子元素上。
...
看步骤页里每步的分值，或导出 JSON 的 `selectorScore`：

| 分值 | 含义 |
|---|---|
| 88–100 | `data-testid` / 稳定 `id` / `name`，改版基本不受影响 |
| 60–87 | 语义属性（`aria-label`、`placeholder`），一般能扛住改版 |
| < 60 | 只能靠文本或结构路径，步骤页会标 **⚠ 易失效** |

大量 ⚠ 说明目标页面几乎没有可用的稳定标识。这种情况下最有效的办法不是调工具，是让那几个关键元素加上 `data-testid`。
